  ...
  return (
    <AppShell>
      <div className="p-4 md:p-6">
        <h1 className="text-xl md:text-2xl font-bold mb-4 md:mb-6">提问与搜索</h1>

        <Tabs defaultValue="ask" className="w-full space-y-6">
          <div className="space-y-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">选择模式</p>
          ...
          <TabsContent value="ask" className="mt-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">询问你的知识库 (测试版)</CardTitle>
                <p className="text-sm text-muted-foreground">
                  LLM将基于你知识库中的文档回答你的问题。
                </p>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">按 Cmd/Ctrl+Enter 提交</p>
                </div>
                ...
                {!hasEmbeddingModel ? (
                  <div className="flex items-center gap-2 p-3 text-sm text-amber-600 dark:text-amber-500 bg-amber-50 dark:bg-amber-950/20 rounded-md">
                    ...
                    <span>你无法使用此功能，因为没有选择嵌入模型。请在模型页面中设置。</span>
                  </div>
                ) : (
                  <>
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2 text-xs flex-wrap">
                        <Badge variant="secondary">
                          策略: {resolveModelName(customModels?.strategy || modelDefaults?.default_chat_model)}
                        </Badge>
                        <Badge variant="secondary">
                          回答: {resolveModelName(customModels?.answer || modelDefaults?.default_chat_model)}
                        </Badge>
                        <Badge variant="secondary">
                          最终: {resolveModelName(customModels?.finalAnswer || modelDefaults?.default_chat_model)}
                        </Badge>
                      </div>
                    </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">按 Enter 搜索</p>
                </div>
                  ...
                  <div className="mt-6 space-y-3">
                    <div className="flex items-center justify-between">
                      <h3 className="text-sm font-medium">
                        找到 {searchMutation.data.total_count} 个结果
                      </h3>
                      <Badge variant="outline">{searchMutation.data.search_type === 'text' ? '文本' : '向量'}搜索</Badge>
                    </div>

                    {searchMutation.data.results.length === 0 ? (
                      <Card>
                        <CardContent className="pt-6 text-center text-muted-foreground">
                          未找到 &ldquo;{searchQuery}&rdquo; 的结果
                        </CardContent>
                      </Card>
                    ) : (
