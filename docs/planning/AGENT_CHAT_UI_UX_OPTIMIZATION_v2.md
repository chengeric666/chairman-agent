...
| # | 问题 | 根本原因 | 状态 |
|---|------|---------|--------|
| **0** | **研究进行中有2个重复的研究过程UI** | **缺少状态机，两个条件分支独立渲染** | **✅ 已修复** |
...
**当前状态**：所有 0-7 问题均已修复，包括核心问题 **问题 0：研究进行中有 2 个重复的研究过程 UI**
...
**步骤**:
1. [x] 重新设计 `SupervisorMessagesContainer` 组件，添加 `phase` 属性
2. [x] 实现 3 种状态的不同 UI 渲染：`starting` / `researching` / `completed`
3. [x] 简化渲染逻辑：移除两个独立条件，改为统一的状态机计算 (Line 569-580)
4. [x] 调整渲染位置：进行中放末尾，完成后放报告之前
